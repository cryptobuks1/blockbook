...
func (s *PublicServer) apiAddress(r *http.Request, apiVersion int) (interface{}, error) {
	...
	page, pageSize, details, filter, _, _, noHex := s.getAddressQueryParams(r, api.AccountDetailsTxidHistory, txsInAPI)
	address, err = s.api.GetAddress(addressParam, page, pageSize, details, filter, noHex)
	if err == nil && apiVersion == apiV1 {
